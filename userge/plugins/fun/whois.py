...
async def who_is(message: Message):
    ...
    if from_user or from_chat is not None:
        ...
        message_out_str = "<b>ㅤㅤㅤㅤ🎖ㅤUSERㅤINFOㅤ🎖</b>\n\n"
        message_out_str += f"<b>╭─🎭 FirstName:</b> <code>{from_user.first_name}</code>\n"
        message_out_str += f"<b>├─🎭 LastName:</b> <code>{from_user.last_name}</code>\n"
        message_out_str += f"<b>├─🎖 Username:</b> @{from_user.username}\n"
        message_out_str += f"<b>├─🏬 DC ID:</b> <code>{from_user.dc_id}</code>\n"
        message_out_str += f"<b>├─🤖 IsBot:</b> <code>{from_user.is_bot}</code>\n"
        message_out_str += f"<b>├─🚫 IsRestricted:</b> <code>{from_user.is_scam}</code>\n"
        message_out_str += "<b>├─✅ IsVerifiedByTelegram:</b> "
        message_out_str += f"<code>{from_user.is_verified}</code>\n"
        message_out_str += f"<b>├─🥷 User ID:</b> <code>{from_user.id}</code>\n"
        message_out_str += f"<b>├─📂 ProfilePhotos:</b> <code>{pp_c}</code>\n"
        try:
            cc_no = len(await message.client.get_common_chats(from_user.id))
        except BotMethodInvalid:
            pass
        else:
            message_out_str += f"<b>├─💭 CommonChats:</b> <code>{cc_no}</code>\n"
        message_out_str += f"<b>╰─📮 Bio:</b> <code>{from_chat.bio}</code>\n\n"
        message_out_str += f"<b>╭─👀 LastSeen:</b> <code>{from_user.status}</code>\n"
        message_out_str += "<b>╰─🏮 PermanentLinkToProfile:</b> "
